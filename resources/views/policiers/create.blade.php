...
@section('title', 'Ajouter un policier')
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
    <div>
        <h1 class="text-xl font-bold text-slate-800">Ajouter un policier</h1>
        <p class="text-sm text-slate-500 mt-0.5">Renseignez les informations du nouveau policier</p>
    </div>
...
<form method="POST" action="{{ route('policiers.store') }}">
@csrf
    ...
    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div>
            ...
            <select name="statut"
                ...
                <option value="{{ $case->value }}" {{ old('statut') === $case->value ? 'selected' : '' }}>
                    {{ $case->label() }}
                </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="date" name="date_prise_service" value="{{ old('date_prise_service') }}"
                   class="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2">
        ...
        <div class="md:col-span-2">
            <label class="inline-flex items-center gap-3 cursor-pointer">
                ...
                <input type="checkbox" name="proprietaire_logement" value="1"
                       {{ old('proprietaire_logement') ? 'checked' : '' }}
