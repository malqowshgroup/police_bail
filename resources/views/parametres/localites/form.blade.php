<div class="flex items-center justify-between gap-3 mb-6">
    <div>
        <div class="flex items-center gap-2 text-xs text-slate-400 mb-1">
            <a href="{{ route('parametres.localites.index') }}" class="hover:text-orange-500">Localités</a>
            <i class="ti ti-chevron-right"></i><span>{{ $titre }}</span>
        </div>
        <h1 class="text-2xl font-bold text-slate-800">{{ $titre }}</h1>
    </div>
    <a href="{{ route('parametres.localites.index') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium text-slate-600 bg-gray-100 hover:bg-gray-200 transition"><i class="ti ti-arrow-left text-sm"></i> Retour</a>
</div>

<form method="POST" action="{{ $action }}" class="max-w-2xl">
@csrf
@if($localite) @method('PUT') @endif

<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
        <div>
            <label class="block text-xs font-semibold text-slate-600 mb-1.5">Code</label>
            <input type="text" name="code" value="{{ old('code', $localite->code ?? '') }}" placeholder="Ex: ABJ"
        ...
        <div>
            ...
            <input type="text" name="libelle" value="{{ old('libelle', $localite->libelle ?? '') }}" placeholder="Ex: Abidjan"
        ...
        <div class="md:col-span-2">
            <label class="inline-flex items-center gap-3 cursor-pointer">
                ...
                <input type="checkbox" name="actif" value="1" {{ old('actif', $localite->actif ?? true) ? 'checked' : '' }} class="w-4 h-4 rounded accent-orange-500">
                <span class="text-sm font-medium text-slate-700">Localité active</span>
            </label>
        </div>
    </div>
    <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
        <a href="{{ route('parametres.localites.index') }}" class="px-5 py-2.5 rounded-xl text-sm font-medium text-slate-600 bg-white border border-gray-200 hover:bg-gray-100 transition">Annuler</a>
