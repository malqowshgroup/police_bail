...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
    <div>
        <h1 class="text-xl font-bold text-slate-800">Ajouter un propriétaire</h1>
        <p class="text-sm text-slate-500 mt-0.5">Renseignez les informations du nouveau propriétaire</p>
    </div>
...
<form method="POST" action="{{ route('proprietaires.store') }}"
      x-data="{ type: '{{ old('type_personne', 'physique') }}' }">
@csrf
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div class="flex gap-6">
                <label class="inline-flex items-center gap-2.5 cursor-pointer">
                    <input type="radio" name="type_personne" value="physique"
                           x-model="type"
                           {{ old('type_personne', 'physique') === 'physique' ? 'checked' : '' }}
                ...
                <label class="inline-flex items-center gap-2.5 cursor-pointer">
                    <input type="radio" name="type_personne" value="morale"
                           x-model="type"
                           {{ old('type_personne') === 'morale' ? 'checked' : '' }}
                           class="accent-orange-500 w-4 h-4">
        ...
        <div>
            ...
            <input type="text" name="num_piece_identite" value="{{ old('num_piece_identite') }}"
        ...
        <div>
            ...
            <select name="type_piece"
                ...
                <option value="sejour" {{ old('type_piece') === 'sejour' ? 'selected' : '' }}>Titre de séjour</option>
            </select>
            @error('type_piece')
            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <input type="text" name="adresse_postale" value="{{ old('adresse_postale') }}"
                   class="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2"
        ...
        <div>
            ...
            <input type="text" name="num_compte_contribuable" value="{{ old('num_compte_contribuable') }}"
        ...
        <div>
            ...
            <select name="localite_id"
                ...
                <option value="{{ $localite->id }}" {{ old('localite_id') == $localite->id ? 'selected' : '' }}>
                    {{ $localite->libelle }}
                </option>
                @endforeach
            </select>
    ...
    <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-3">
        ...
        <button type="submit"
            ...
            <i class="ti ti-device-floppy mr-1"></i> Enregistrer
        </button>
    </div>
