...
class StoreContratBailRequest extends FormRequest
{
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator) {
            // Un policier ne peut avoir qu'un seul contrat en cours.
            if ($this->policier_id && $this->contratEnCoursExiste('policier_id', $this->policier_id)) {
                $validator->errors()->add('policier_id', 'Ce policier possède déjà un contrat de bail en cours.');
            }

            // Un logement ne peut être lié qu'à un seul contrat en cours.
            if ($this->logement_civil_id && $this->contratEnCoursExiste('logement_civil_id', $this->logement_civil_id)) {
                $validator->errors()->add('logement_civil_id', 'Ce logement est déjà rattaché à un contrat de bail en cours.');
            }
        });
    }

    private function contratEnCoursExiste(string $colonne, int $valeur): bool
    {
